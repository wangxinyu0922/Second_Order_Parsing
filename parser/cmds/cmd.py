...
class CMD(object):
    ...
    def decode(self, arc_scores, rel_scores, mask):
        ...
        if self.args.tree and not self.args.mst:
            arc_preds = eisner(arc_scores, mask)
        elif self.args.mst:
            # since the input of mask has been masked at position 0, mapping the root_mask to 1
            root_mask = mask.clone()
            root_mask[:,0] = 1
            lens = root_mask.sum(1)
            binary_mask = root_mask.unsqueeze(-2) * mask.unsqueeze(-1)
            masked_arc_scores = arc_scores.masked_fill(~binary_mask.bool(), float(-1e9))
            # arc_preds[idx]=MST_inference(torch.softmax(masked_arc_scores[0],-1).cpu().numpy(), lens[0].cpu().numpy(), binary_mask[0].cpu().numpy())
            # MST_inference(torch.softmax(arc_scores[1],-1).cpu().numpy(), lens[1].cpu().numpy(), binary_mask[1].cpu().numpy())
            # comp=eisner(arc_scores, mask)
            arc_preds=[]
            for idx in range(len(arc_scores)):
                arc_preds.append(torch.Tensor(MST_inference(torch.softmax(masked_arc_scores[idx],-1).cpu().numpy(), lens[idx].cpu().numpy(), binary_mask[idx].cpu().numpy())).type_as(arc_scores))
            arc_preds=torch.stack(arc_preds,0).long()
        else:
